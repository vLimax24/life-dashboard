...
export function ExamModal({ open, onClose, onSaved }: Props) {
  ...
  const save = () => {
    if (!subject || !date) { onSaved("Bitte Fach und Datum ausfüllen"); return; }
    const plan = generatePrepPlan(date, type, topic);
    const exams = S.get<ExamPlan[]>("exam_plans", []);
    exams.push({ id: Date.now(), subject, date, type, topic, createdAt: getToday(), plan });
    S.set("exam_plans", exams);
    const events = S.get<Event[]>("events", []);
    events.push({ id: Date.now() + 1, title: subject + (type === "klausur" ? " Klausur" : " Test"), date, type: type === "klausur" ? "exam" : "test", subject });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Lernplan erstellen">
      ...
      <FormGroup label="Typ">
        <FormSelect value={type} onChange={e => setType(e.target.value as "klausur" | "test")}>
      ...
      <FormGroup label="Thema / Stoff">
        <FormInput placeholder="z.B. Differentialrechnung, Kapitel 3-5" value={topic} onChange={e => setTopic(e.target.value)} />
      </FormGroup>
